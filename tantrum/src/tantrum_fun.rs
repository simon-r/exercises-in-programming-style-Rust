...
fn read_and_filter_words(
    ...
    let mut stop_words_str = String::new();

    match &stop_words {
        Ok(s) => stop_words_str.clone_from(s),
        Err(_s) => {
    ...
    let re_sw = Regex::new(r"^[\w+,]");

    if !re_sw.ok().unwrap().is_match(&stop_words_str) {
        return Err(String::from("Badly formatted stop words"));
    }
...
fn frequencies_to_string(vec_str: &Vec<String>) -> Result<String, String> {
    if vec_str.len() == 0 {
        return Err(String::from("Empty dictionary"));
    }
    ...
    Ok(vs.iter().fold(String::new(), |mut st, el| {
        if el.1 > 4 {
            st.push_str(&format!("{} - {}\n", el.0, el.1));
        }
        st
...
//////////////////////////////////////////////////////////////////
pub fn tantrum_test(file_name: &String, file_stop_w: &String) {
